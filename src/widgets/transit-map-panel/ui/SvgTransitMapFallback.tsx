...
const SvgTransitMapFallback = React.memo(function SvgTransitMapFallback({
  startStation,
  endStation,
  onSelectStation,
  selectedPlan,
  ...
  useEffect(() => {
    if (selectedPlan && startNode && endNode) {
      ...
      let minX = Math.min(startNode.x ?? 0, endNode.x ?? 0);
      let maxX = Math.max(startNode.x ?? 0, endNode.x ?? 0);
      let minY = Math.min(startNode.y ?? 0, endNode.y ?? 0);
      let maxY = Math.max(startNode.y ?? 0, endNode.y ?? 0);
      ...
      const mid1X = startX + (endX - startX) * 0.35 + (selectedPlan.id === "plan_a" ? 30 : -25);
      const mid1Y = startY + (endY - startY) * 0.25 + (selectedPlan.id === "plan_b" ? -35 : 15);
      const mid2X = startX + (endX - startX) * 0.75 + (selectedPlan.id === "plan_a" ? -15 : 20);
      const mid2Y = startY + (endY - startY) * 0.80 + (selectedPlan.id === "plan_b" ? 25 : -15);
  ...
  const getPathSegments = () => {
    ...
    let segmentsString = `M ${startX} ${startY}`;
    ...
    segmentsString += ` Q ${mid1X} ${mid1Y}, ${mid2X} ${mid2Y} T ${endX} ${endY}`;
          ...
          stations
            ...
            .map((s, idx) => (
              <g key={`bus-pos-${idx}`} opacity="0.95" className="drop-shadow-lg">
                <circle cx={(s.x ?? 0) - 18} cy={(s.y ?? 0) - 12} r="8" fill="#0A84FF" />
                <circle cx={(s.x ?? 0) - 18} cy={(s.y ?? 0) - 12} r="9" fill="none" stroke="#FFFFFF" strokeOpacity="0.4" strokeWidth="1.5" />
                <text
                  x={(s.x ?? 0) - 18}
                  y={(s.y ?? 0) - 9}
                  fill="#FFF"
          ...
          return (
            ...
            >
              ...
              {visibleLayers.bike && station.bikesAvailable && (
                <g transform={`translate(${(station.x ?? 0) + 12}, ${(station.y ?? 0) + 4})`}>
                  <rect
                    width="22"
                    height="11"
                    rx="3"
...
export default SvgTransitMapFallback;
